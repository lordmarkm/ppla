            ...
            compile: function (element) {
                ...
                return function (scope, element, attrs) {
                    ...
                    if (!element.hasClass('ng-table')) {
                        ...
                        var paginationTemplate = angular.element('<tfoot />').append(angular.element('<tr />').append(angular.element('<td class="no-padder" />').attr({ 'ng-include': 'templates.pagination', 'colspan': columns.length })));
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager.html', '<footer class="panel-footer ng-cloak ng-table-pager"><div class="row"><div class="col-xs-5"><div class="text-muted text-sm" ng-if="params.settings().total===0">No entries found.</div><select ng-show="params.settings().counts.length && params.settings().total > 10" class="ng-table-counts form-control input-sm pull-left" ng-model="countPerPage" ng-change="params.count(countPerPage)" ng-init="countPerPage=params.count()"><option ng-repeat="count in params.settings().counts">{{count}}</option></select><span class="text-muted inline"><small ng-if="params.settings().total>0" class="text-muted text-ellipsis m-t-sm m-l-xs">Showing {{params.data.length===params.count() ? ((params.page() * params.data.length) - params.data.length) + 1 : (params.settings().total - params.data.length) + 1}} to {{params.data.length===params.count() ? params.page() * params.data.length : params.settings().total}} of {{params.settings().total}} entries</small></span></div><div class="col-xs-7 text-right text-center-xs"><ul class="pagination pagination-sm m-t-none m-b-none"><li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"><a ng-switch-when="prev" ng-click="params.page(page.number)" href=""><i class="fa fa-chevron-left"></i></a><a ng-switch-when="first" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a><a ng-switch-when="page" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a><a ng-switch-when="more" ng-click="params.page(page.number)" href=""><i class="fa fa-ellipsis-h"></i></a><a ng-switch-when="last" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a><a ng-switch-when="next" ng-click="params.page(page.number)" href=""><i class="fa fa-chevron-right"></i></a></li></ul></div></div></footer>');
}]);
    return app;
}));